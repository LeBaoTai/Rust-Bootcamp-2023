...
impl StateMachine for Atm {
    ...
    fn next_state(starting_state: &Self::State, t: &Self::Transition) -> Self::State {
        ...
        let mut keys = starting_state.keystroke_register.to_owned();

        match atm.expected_pin_hash {
            Auth::Waiting => {
                match t {
                    Action::SwipeCard(pass) => {
                        atm.expected_pin_hash = Auth::Authenticating(*pass);
                        atm.keystroke_register = keys.clone();
                    },
                    _ => {
            ...
            Auth::Authenticating(pass) => {
                match t {
                    ...
                    Action::PressKey(k) => {
                        match k {
                            Key::Enter => {
                                let hashed_key = crate::traits::hash(&keys);
                                if hashed_key == pass {
                    ...
                    Action::PressKey(k) => {
                        match k {
                            Key::Enter => {
                                ...
                                for key in keys {
                                    amount *= 10;
                                    amount += <Key as Into<u64>>::into(key);
                                }
